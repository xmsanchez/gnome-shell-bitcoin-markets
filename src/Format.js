...
var format = (value, { base, quote, format }) => {
  ...
  const formatValueWithDigits = (value, scale, digits) => {
    ...
    return (value * scale).toLocaleString(
      undefined /* locale */, {
  ...
  const scale = [["", 1], ["m", 0.001], ["k", 1000], ["sat", 1e8]];
  scale.forEach(([prefix, scale]) => {
    formatData[`${prefix}v`] = formatValueWithDigits(
      value, scale, info ? info.decimal_digits : defaultDigits
    );
    for (let i = 0; i < 9; i++) {
      formatData[`${prefix}v${i}`] = formatValueWithDigits(
        value, scale, i
      );
    }
  });

  return StringFormat.format(format, formatData);
}
